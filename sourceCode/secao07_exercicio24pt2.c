...
foram marcadas em negrito. O produto desses númreos é 26*63*78*14 = 1788696.
...
int main()
{
    ...
    FILE *archivo;
    archivo = fopen("matriz_secao07_exercicio24pt2.txt","r");
    delay(5);  //Coloquei um delay para ver se abre o arquivo.

    if (archivo == NULL)    //Em debug mode, o programa funciona, mas em run mode, não carrega o arquivo
    {
    ...
    printf("\n");
    printf("Matriz importada do arquivo:\n");
    for (i = 0; i < linhas; i++)
    {
        for (j = 0; j < colunas; j++)
            printf("%d ", matriz[i][j]);
